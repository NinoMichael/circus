...
const UseConditions = () => {
    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            ...
            <div className="mt-8 mx-36 max-xs:mx-16 max-xs:text-sm">
                <h1 className="font-kanit text-3xl max-sm:text-center">{t('conditionTitle')}</h1>
                <p className='mt-4 max-sm:text-center'>{t("conditionP")}</p>

                {threeFirst.map((item, index) => (
                    <article key={index} className="mt-8 mb-4 max-sm:text-center">
