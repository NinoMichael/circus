...
const DisableAccountVisitor = () => {
	const navigateTo = useNavigate();
	const { deactivate, loading } = useAuth();
	...
	async function handleSubmit(e: React.FormEvent) {
		...
		const success = await deactivate(password);
		...
		>
			<form className="flex flex-col mx-auto max-w-3xl" onSubmit={handleSubmit}>
				<div className="space-y-2">
					<h1 className="text-2xl font-extrabold">Désactiver mon compte</h1>
					<span className="opacity-60">
						Nous sommes désolés de vous voir partir. La désactivation de votre
						compte Circus est une action importante qui nécessite votre
						attention
					</span>
				</div>
				...
				<div className="mt-8 space-y-3 bg-primary/20 rounded-lg p-6 text-text/70 border border-l-4 border-primary/60 shadow">
					<h4>
						...
						<span className="text-lg font-bold">Avertissement</span>
					</h4>
					<p>
						La désactivation de votre compte est une action sérieuse. Votre
						profil sera immédiatement suspendu et vous ne pouvez pas accéder aux
						services exclusifs de Circus
					</p>
				</div>
				...
				<div className="mt-8">
					...
					<div className="mt-4 grid gap-4">
						<div className="bg-white rounded-lg shadow p-6 flex gap-3 items-center">
							...
							<span>Invalidation de tous les billets et pass actifs</span>
						</div>
						<div className="bg-white rounded-lg shadow p-6 flex gap-3 items-center">
							<HistoryIcon className=" text-primary bg-primary/20 p-2 rounded-full size-10" />
							<span>Données conservées temporairement</span>
						</div>
					</div>
				</div>
...
export default DisableAccountVisitor;
