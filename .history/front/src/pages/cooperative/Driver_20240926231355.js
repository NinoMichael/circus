...
const DriverCoop = () => {
    ...
    useEffect(() => {
        const fetchChauffeurs = async () => {
            const response = await getChauffeurs(); // Récupérez les données de l'API
            // Transformation des données
            const transformedData = response.map(chauffeur => ({
                id: chauffeur.id_chauffeur,
                nom: chauffeur.nom_chauffeur,
                age: chauffeur.age || 32, // Ajustez selon votre logique
                statut: chauffeur.statut || 'Disponible', // Ajustez selon vos données
                img: imgDriver,
            }));
