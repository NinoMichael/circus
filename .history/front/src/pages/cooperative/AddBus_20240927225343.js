            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="ps-[16.25rem] -z-50">
                ...
                <main className="mt-3">
                    ...
                    <section className="grid grid-cols-2 mt-8">
                        <FileUpload ref={fileUploadRef} mode="basic" name="demo[]" accept="image/*" maxFileSize={1000000} customUpload auto uploadHandler={onUpload} className="invisible" />
                        <div className="border border-dashed border-black py-20 px-12 w-64 m-auto flex justify-center items-center">
                            {imgTransport ?
                                <img src={imgTransport} alt="uploaded" /> :
                                <p className="border border-dashed text-sm flex justify-center items-center m-auto cursor-pointer" onClick={() => fileUploadRef.current.choose()}>
                                    <i className="pi pi-plus me-2"></i>{t('addImage')}
                                </p>
                            }
                        </div>
