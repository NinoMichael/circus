    ...
    const renderGridBus = (bus, index) => {
        return (
            <div className="col-12 bg-white border shadow-sm rounded" key={index}>
                ...
                <div className="p-4">
                    <h4 className="font-poppins text-black text-sm"><i className = "pi pi-car me-2"></i>{bus.matricule}</h4>
                    <p className = "font-poppins text-black text-sm font-thin"><i className = "pi pi-user me-2"></i>{bus.chauffeur}</p>
                </div>
  
                <div className="flex justify-between">
                    <Link to = ""><Button className="bg-white border border-none outline outline-none text-sm text-gray-600 mx-4" icon = "pi pi-check" /></Link>
                    <Link to = ""><Button className="bg-white border border-none outline outline-none text-sm text-gray-600 mx-4" icon = "pi pi-trash" /></Link>
                </div>
            </div>
        );
    };
