...
const BusCoop = () => {
        ...
        return (
            <div className="col-12 bg-white border shadow-sm rounded" key={index}>
                <div className = "relative">
                    <img src = {bus.img} alt = "Bus" width = {150} height = {150} className="w-full h-48 rounded-t"/>
                    <Tag value={bus.status} severity={getSeverity(bus)}></Tag>
                </div>
            </div>
        );
    };
    ...
    const listTemplate = (items) => {
        ...
        return <div className="grid grid-cols-4 grid-nogutter">{list}</div>;
    };
